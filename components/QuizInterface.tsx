...
export default function QuizInterface({ topic, onBack, apiKey, model, baseUrl, onShowOptions }: QuizInterfaceProps) {
  ...
  // Common Context Popup component
  const ContextPopup = () => {
    if (!showContextPopup || !currentQuestion) return null;
  ...
  return (
    <div className="quiz-wrapper">
          ...
          {!checkResult && !skipped && (
            <div className="flex gap-4 mb-4">
              <input
                ...
                className="input py-3 text-lg flex-1 min-w-0"
                autoComplete="off"
                spellCheck="false"
                autoFocus
              />
      ...
      {/* Context Popup moved outside the card to avoid overflow issues */}
      <ContextPopup />
    </div>
  );
}